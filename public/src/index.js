...
const firebaseConfig = {
  apiKey: 'AIzaSyDtFO8VGzz3nMCfL2MVJUn5d2dPNH6cMFc',
  authDomain: 'mine-cad.firebaseapp.com',
  projectId: 'mine-cad',
  storageBucket: 'mine-cad.appspot.com',
  messagingSenderId: '94815448038',
  appId: '1:94815448038:web:a35e76e1e4fca1c6174427',
  measurementId: 'G-XDWPGFL3Z5',
}
...
if (WEBGL.isWebGLAvailable()) {
  ...
  let container = document.querySelector(
    '#left_container.edit_window_container'
  )
  ...
  for (let [block_id, block_info] of Object.entries(blockInfo)) {
    ...
    button.addEventListener('click', (e) => {
      if (Blocks[e.target.id]) cadInfo.curBlock = new Blocks[e.target.id](50)
      else if (Blocks[blockInfo[e.target.id].type]) {
        cadInfo.curBlock = new Blocks[blockInfo[e.target.id].type](
          e.target.id,
          50,
          { axis: 1 }
        )
        ...
        for (let z = 0; z < cadInfo.mapSize; z++) {
          if (cadInfo.blueprint[y][x][z]) {
            ...
            if (attrInfo[blockInfo[cadInfo.blueprint[y][x][z].id].type]) {
              text += '['
              for (let [attr] of Object.entries(
                attrInfo[blockInfo[cadInfo.blueprint[y][x][z].id].type]
              )) {
  ...
  function init() {
    ...
    let gridHelper = new THREE.GridHelper(
      cadInfo.blockSize * cadInfo.mapSize,
      cadInfo.mapSize
    )
      ...
      case 1: //left button
        if (intersects.length > 0) {
          ...
          if (intersect.object !== plane) {
            let location = intersect.object.position
              .addScalar(-25)
              .divideScalar(50)
            cadInfo.blueprint[location.y][location.x + cadInfo.mapSize / 2][
              location.z + cadInfo.mapSize / 2
            ] = null
